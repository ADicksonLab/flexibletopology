...
PLUGINS_PATH = '/Users/nazanin/anaconda3/pkgs/openmm-7.4.2-py37_cuda101_rc_1/lib/plugins'
...
def create_mlforce(ghost_particle_idxs):
    start_idx = 1227
    mlmodel_name = 'anigsg_model.pt'
    data_file_name = f'T{start_idx}_W4_110_anigsg.pkl'
...
if __name__ == '__main__':
    ...
    # add mlforce
    ghost_particle_idxs = [i+n_part_system for i in range(n_ghosts)]
    ex_mlforce = create_mlforce(ghost_particle_idxs)
    ex_mlforce.setForceGroup(30)
    system.addForce(ex_mlforce)
    ...
    for iteration in range(n_iter):
        ...
        gsforce_state = simulation.context.getState(getEnergy=True, groups={30})
        gsg_energy = gsforce_state.getPotentialEnergy() / unit.kilocalorie_per_mole
        print(f'GSG energy: {gsg_energy: .3f}')
