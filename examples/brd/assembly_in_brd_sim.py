...
REPORT_STEPS = 1
PLATFORM = 'CUDA'
COM_BS = [[4.1221001,3.2817999,2.87659995]] #in nm
WIDTH = 0.3 #in nm
TOL= 100
MAXITR = 1000
...
TARGET_IDX = 124
OUTPUTS_PATH = f'outputs_2/T{TARGET_IDX}'
...
#SB:
#Nazanin needed this to keep the ghosts connected via the custom compound bond force
#Will our system need this? I think Yes.
def writeBondEnergyString(n_ghosts):
    # write main energy expression
    energy_string = "0.5*k*("

    for i in range(1, n_ghosts+1):
        energy_string += f'step(d{i}-d0)*(d{i}-d0)^2'
        if i < n_ghosts:
            energy_string += ' + '
    energy_string += ');'
    ...
    for i in range(1, n_ghosts+1):
        energy_string += f' d{i}=sqrt((x{i}-cx)^2 + (y{i}-cy)^2 + (z{i}-cz)^2);'
...
if __name__ == '__main__':
    ...
    signal_force_weights = [1.0, 1.0, 1.0, 1.0]
    ...
    #ccbf = omm.CustomCompoundBondForce(
    #   n_ghosts, writeBondEnergyString(n_ghosts))
    #ccbf.addGlobalParameter('k', 500)
    #
    #ccbf.addGlobalParameter('d0', 1.5)
    ...
    for gh_idx in range(n_ghosts):
        ...
        # creating the force
        CCB_force = omm.CustomCentroidBondForce(
                    2,"step(distance(g1,g2)-CCB_CUTOFF_DIST)*0.5*k*distance(g1,g2)^2")
    ...
    simulation.reporters.append(mdj.reporters.DCDReporter(osp.join(OUTPUTS_PATH, SIM_TRAJ),
                                                          REPORT_STEPS))
    print('Minimization of the full system (including the protein)')
    ...
    #we can write the position_em to a pdb file as well
