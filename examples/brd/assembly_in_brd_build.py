...
def init_positions(COM_BS, WIDTH, n_ghosts, min_dist):
    positions = []
    positions.append(COM_BS)
    while len(positions) < n_ghosts:
        r_pos = np.random.uniform(low=-WIDTH, high=WIDTH,size=(1, 3))        
        r_pos = r_pos+COM_BS
        distances = np.linalg.norm(np.concatenate(positions) - r_pos,
                                   axis=1)
...
if __name__ == '__main__':
    ...
    omma.PDBFile.writeFile(psf.topology, position_em, open('minim_'+str(n_ghosts)+'.pdb','w'))
