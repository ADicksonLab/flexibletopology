...
INIT_BOUNDS = {'charge': (-0.2, 0.2),
               'sigma': (sigma_LowerBound, sigma_LowerBound), 
               'epsilon': (0.03, 1.50),
               'lambda': (1.0, 1.0)}

SIM_BOUNDS = {'charge': (-0.90, 0.90),
...
if __name__ == '__main__':
    ...
    BUILD_UTILS = SystemBuild(psf=psf, crd=crd, pdb=pdb_file, n_ghosts=n_ghosts,
                              ...
                              ghost_mass=GHOST_MASS, attr_bounds=INIT_BOUNDS,
                              contForce=con_force)
    ...
    integrator = CustomHybridIntegratorRestrictedChargeVariance(n_ghosts, TEMPERATURES[0], FRICTION_COEFFICIENT,
                                                TIMESTEP, attr_fric_coeffs=coeffs, attr_bounds=SIM_BOUNDS)
    ...
    with open("init_attr.txt","w") as f:
        for p in par:
            print(p[0],p[1],p[2],p[3],file=f)
